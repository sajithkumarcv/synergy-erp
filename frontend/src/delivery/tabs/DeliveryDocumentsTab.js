...
// ─────────────────────────────────────────────────────────────────────
const DeliveryDocumentsTab = ({ delivery }) => {
    const currentUser = useCurrentUser();
    const { canDo }   = usePermission();
    const fileRef     = useRef(null);

    // Editable unless Delivered or Cancelled
    const editable  = !['Delivered', 'Cancelled'].includes(delivery?.status)
                      && canDo('/delivery', 'EDIT');
    ...
    const load = useCallback(() => {
        setLoading(true);
        fetch(`${variables.API_URL}document/DN/${delivery.deliveryId}`, { headers: authHeaders() })
            .then(r => r.json())
    ...
    useEffect(() => {
        fetch(`${variables.API_URL}document/types/DN`, { headers: authHeaders() })
            .then(r => r.ok ? r.json() : [])
            .then(d => {
    ...
    const upload = () => {
        ...
        fetch(`${variables.API_URL}document/DN/${delivery.deliveryId}`, {
            method: 'POST',
    ...
    const missingMandatory = types.filter(t =>
        t.isMandatory && !docs.some(d => d.documentType === t.documentType)
    );
            ...
            {missingMandatory.length > 0 && (
                <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, padding: '8px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12.5, color: '#9a3412' }}>
                        ...
                        {missingMandatory.map(t => t.documentType).join(', ')}
                    </span>
                </div>
            )}
            ...
            {!editable && (
                <div style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 8, padding: '8px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12.5, color: '#475569' }}>
                        This delivery note is locked. Documents cannot be uploaded or deleted.
                    </span>
                </div>
            )}
            ...
            {editable && (
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: 16, marginBottom: 20 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', flex: '0 0 220px' }}>
                            <label style={{ fontSize: 10.5, fontWeight: 600, color: '#64748b', marginBottom: 4 }}>Document Type *</label>
                            <select className="pf-input" value={form.documentType}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 180 }}>
                            ...
                            <input ref={fileRef} type="file" accept=".jpg,.jpeg,.pdf"
                                className="pf-input" style={{ padding: '5px 8px', cursor: 'pointer' }}
                                onChange={handleFile} />
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 140 }}>
                            ...
                            <input className="pf-input" placeholder="Optional note…"
                                value={form.remarks}
                                onChange={e => setForm(p => ({ ...p, remarks: e.target.value }))} />
                        </div>

                        <button className="pf-btn-pri" onClick={upload}
                        ...
                        return (
                            <div key={doc.documentId} style={{
                                ...
                                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                                    ...
                                    {editable && (
                                        <button
                                            style={{ padding: '5px 10px', fontSize: 12, background: '#fee2e2', color: '#991b1b', border: '1px solid #fca5a5', borderRadius: 6, cursor: 'pointer' }}
                                            onClick={() => del(doc)}>
                                            🗑
                                        </button>
                                    )}
...
export default DeliveryDocumentsTab;
