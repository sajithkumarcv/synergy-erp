...
// ── Simple status badge (no document-status config for CN) ───────────────
const STATUS_COLORS = {
...
const exportCsv = (rows, baseCcy) => {
    ...
    const headers = ['#','CN Number','Date','Customer','Curr','Rate','Credit Type',
        'Credit','Credit ('+base+')','Allocated ('+base+')','Unallocated ('+base+')',
    ...
    const lines = [
        headers.join(','),
        ...rows.map((r, i) => [
            i + 1, r.cnNumber, r.cnDate ? fmtDate(r.cnDate) : '',
            r.customerName, r.currencyShort, r.exchangeRate, r.creditType,
            r.creditAmount, r.creditAmountBase, r.allocatedBase, r.unallocatedBase,
    ...
    a.href = url; a.download = `CreditNote_Report_${today()}.csv`; a.click();
...
const CreditNoteReport = () => {
    ...
    const runReport = useCallback(async () => {
        ...
        if (filters.creditType) p.set('creditType', filters.creditType);
        if (filters.createdBy)  p.set('createdBy',  filters.createdBy);
        try {
            const res  = await fetch(`${variables.API_URL}reports/credit-note?${p}`, { headers: authHeaders() });
    ...
    const totals = useMemo(() => {
        ...
        return {
            count:          rows.length,
            creditBase:     rows.reduce((s, r) => s + (r.creditAmountBase || 0), 0),
    ...
    const currencySummary = useMemo(() => {
        ...
        rows.forEach(r => {
            ...
            if (!map[key]) map[key] = {
                ...
                count: 0, credit: 0, creditBase: 0, allocatedBase: 0, unallocatedBase: 0,
            };
            map[key].count++;
            map[key].credit          += r.creditAmount     || 0;
            map[key].creditBase      += r.creditAmountBase || 0;
    ...
    return (
        <div className="rpt-page">
            ...
            <div className="rpt-header">
                ...
                <div>
                    <div className="rpt-header-title">Credit Note Report</div>
                    <div className="rpt-header-sub">
                        {rows == null
                            ? 'Set filters and click Run Report'
                            : `${rows.length} Credit Note${rows.length !== 1 ? 's' : ''} found`}
                    </div>
                </div>
            ...
            <div className="rpt-filter-card">
                <div className="rpt-filter-row">
                    ...
                    <div className="rpt-filter-group w160">
                        <span className="rpt-filter-label">Credit Type</span>
                        <select className="rpt-filter-select"
                            value={filters.creditType}
                            onChange={e => setF('creditType', e.target.value)}>
                            <option value="">All Types</option>
                            {getVList('CreditNote', 'CreditType').map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                        </select>
                    </div>
            ...
            {totals && (
                <div className="rpt-summary">
                    <div className="rpt-summary-item">
                        <div className="rpt-summary-label">Total CNs</div>
                        <div className="rpt-summary-val">{totals.count}</div>
                    </div>
                    <div className="rpt-summary-item">
                        <div className="rpt-summary-label">Credit ({baseCcy})</div>
                        <div className="rpt-summary-val blue">{fmt(totals.creditBase)}</div>
                    </div>
                    <div className="rpt-summary-item">
                    ...
                    <table className="rpt-ccy-table">
                        <thead>
                            <tr>
                                ...
                                <th className="r">Credit ({baseCcy})</th>
                        ...
                        <tbody>
                            {currencySummary.map(g => (
                                <tr key={`${g.currency}|${g.rate}`}>
                                    ...
                                    <td className="r mono" style={{ color: g.isBase ? '#475569' : '#7c3aed' }}>{fmt(g.credit)}</td>
                                    <td className="r mono" style={{ color: '#6d28d9', fontWeight: 700 }}>{fmt(g.creditBase)}</td>
            ...
            <div className="rpt-content">
                ...
                {!loading && rows !== null && rows.length === 0 && (
                    <div className="rpt-state">
                        <div className="rpt-state-icon">🔍</div>
                        <span>No credit notes match the selected filters.</span>
                    </div>
                )}
                            ...
                            <table className="rpt-table">
                                <thead>
                                    <tr>
                                        ...
                                        <Th col="cnNumber"         label="CN Number"                    />
                                        <Th col="cnDate"           label="Date"                         />
                                        <Th col="customerName"     label="Customer"                     />
                                        <Th col="currencyShort"    label="Curr"                     cls="r" />
                                        <Th col="exchangeRate"     label="Rate"                     cls="r" />
                                        <Th col="creditType"       label="Credit Type"                  />
                                        <Th col="creditAmount"     label="Credit"                   cls="r" />
                                        <Th col="creditAmountBase" label={`Credit (${baseCcy})`}    cls="r" />
                                ...
                                <tbody>
                                    {paged.map((r, i) => {
                                        ...
                                        return (
                                        <tr key={r.cnId} onClick={() => navigate(`/credit-notes/${r.cnId}`)}>
                                            <td className="c" style={{ color: '#94a3b8', fontSize: 11, fontWeight: 600 }}>{rowOffset + i + 1}</td>
                                            <td><span style={{ fontFamily: 'Courier New', fontWeight: 700, color: '#6d28d9', fontSize: 11.5, background: '#ede9fe', padding: '2px 8px', borderRadius: 4 }}>{r.cnNumber}</span></td>
                                            <td style={{ color: '#475569', fontSize: 12 }}>{fmtDate(r.cnDate)}</td>
                                            ...
                                            <td style={{ fontSize: 11.5, color: '#475569' }}>{r.creditType || <span className="muted">—</span>}</td>
                                            <td className="r" style={{ fontFamily: 'monospace', color: isForeign ? '#7c3aed' : '#6d28d9', fontSize: 12 }}>{fmt(r.creditAmount)}</td>
                                            <td className="r" style={{ fontFamily: 'monospace', fontWeight: 700, color: '#6d28d9', fontSize: 12.5 }}>{fmt(r.creditAmountBase)}</td>
                                ...
                                {totals && (
                                    <tfoot>
                                        <tr style={{ background: '#f1f5f9', borderTop: '2px solid #cbd5e1', fontWeight: 700 }}>
                                            ...
                                            <td />
                                            <td style={{ textAlign: 'right', padding: '8px 6px', fontFamily: 'monospace', color: '#6d28d9' }}>{fmt(totals.creditBase)}</td>
