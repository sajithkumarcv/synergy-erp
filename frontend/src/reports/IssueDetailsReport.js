...
const exportCsv = (rows) => {
    ...
    a.download = `IssueDetails_Report_${today()}.csv`;
    a.click();
};
...
const IssueDetailsReport = () => {
    ...
    const runReport = useCallback(async () => {
        ...
        try {
            const res  = await fetch(`${variables.API_URL}reports/issue-details?${p}`, { headers: authHeaders() });
    ...
    return (
        <div className="rpt-page">
            <div className="rpt-header">
                ...
                <div>
                    <div className="rpt-header-title">Issue Details Report</div>
                ...
                <div className="rpt-filter-row">
                    ...
                    <div className="rpt-filter-group">
                        ...
                        <div className="rpt-toggle">
                            <button className={`rpt-toggle-btn ${filters.costingType === '' ? 'active' : ''}`} onClick={() => setF('costingType', '')}>All</button>
                            <button className={`rpt-toggle-btn ${filters.costingType === 'INC_COSTING' ? 'active' : ''}`} onClick={() => setF('costingType', 'INC_COSTING')}>Inc.</button>
                            <button className={`rpt-toggle-btn ${filters.costingType === 'EXC_COSTING' ? 'active' : ''}`} onClick={() => setF('costingType', 'EXC_COSTING')}>Exc.</button>
                        </div>
                    </div>
            ...
            <div className="rpt-content">
                ...
                {!loading && rows !== null && rows.length === 0 && (
                    <div className="rpt-state"><div className="rpt-state-icon">🔍</div><span>No issue lines match the selected filters.</span></div>
                )}
                                ...
                                <tbody>
                                    {paged.map((r, i) => (
                                        <tr key={r.issueLineId} onClick={() => navigate(`/inventory-issue/${r.issueId}`)}>
                                            ...
                                            <td style={{ fontSize: 11.5, color: '#64748b' }}>{r.issuedTo || <span className="muted">—</span>}</td>
                                        </tr>
                                    ))}
                                </tbody>
                                {totals && (
...
export default IssueDetailsReport;
