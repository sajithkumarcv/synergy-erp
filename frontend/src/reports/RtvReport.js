...
const exportCsv = (rows, baseCcy) => {
    ...
    const headers = ['#', 'RTV Number', 'Date', 'Vendor', 'GRN Number', 'PO Number', 'Job',
        'Status', 'Currency', 'Rate', 'Lines', 'Amount', `Amount (${base})`, 'Return Reason', 'Created By'];
    const esc = v => {
        if (v == null) return '';
        const s = String(v);
    ...
    const lines = [
        headers.join(','),
        ...rows.map((r, i) => [
            i + 1, r.rtvNumber, r.rtvDate ? fmtDate(r.rtvDate) : '',
            r.supplierName, r.grnNumber, r.poNumber,
            r.jobId, r.status, r.currencyShort, r.exchangeRate,
            r.lineCount, r.totalAmount, r.totalAmountBase,
            r.returnReason, r.createdBy,
        ].map(esc).join(','))
    ];
    const blob = new Blob([lines.join('\n')], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `RTV_Report_${today()}.csv`; a.click();
    URL.revokeObjectURL(url);
};

// ── Status badge ────────────────────────────────────────────────────────
const StatusBadge = ({ status, getStatusConfig }) => {
    const cfg = statusBadgeCfg(getStatusConfig('RTV', status));
    return (
        <span style={{
...
const RtvReport = () => {
    ...
    const runReport = useCallback(async () => {
        ...
        try {
            const res  = await fetch(`${variables.API_URL}reports/rtv?${p}`, { headers: authHeaders() });
    ...
    const currencySummary = useMemo(() => {
        ...
        rows.forEach(r => {
            ...
            if (!map[key]) map[key] = {
                ...
                count: 0, totalAmount: 0, totalAmountBase: 0,
            };
            map[key].count++;
    ...
    return (
        <div className="rpt-page">
            ...
            <div className="rpt-header">
                <div className="rpt-header-icon" style={{ background: 'linear-gradient(135deg,#ffedd5,#fed7aa)' }}>↩</div>
                <div>
                    <div className="rpt-header-title">Return to Vendor Report</div>
                    <div className="rpt-header-sub">
                        {rows == null
                            ? 'Set filters and click Run Report'
                            : `${rows.length} RTV${rows.length !== 1 ? 's' : ''} found`}
                    </div>
                </div>
            ...
            {totals && (
                <div className="rpt-summary">
                    <div className="rpt-summary-item">
                        <div className="rpt-summary-label">Total RTVs</div>
                    ...
                    <div className="rpt-summary-item">
                        <div className="rpt-summary-label">Draft</div>
                        <div className={`rpt-summary-val ${totals.draft > 0 ? 'amber' : ''}`}>{totals.draft}</div>
                    </div>
                </div>
            )}
                        ...
                        <tbody>
                            {currencySummary.map(g => (
                                <tr key={`${g.currency}|${g.rate}`}>
                                    ...
                                    <td className="r mono" style={{ color: '#c2410c', fontWeight: 700 }}>{fmt(g.totalAmountBase)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <div className="rpt-content">
                ...
                {!loading && rows !== null && rows.length === 0 && (
                    <div className="rpt-state">
                        <div className="rpt-state-icon">🔍</div>
                        <span>No RTVs match the selected filters.</span>
                    </div>
                )}
                            ...
                            <table className="rpt-table">
                                <thead>
                                    <tr>
                                        ...
                                        <Th col="rtvNumber"    label="RTV Number"    />
                                        <Th col="rtvDate"      label="Date"           />
                                ...
                                <tbody>
                                    {paged.map((r, i) => (
                                        <tr key={r.rtvId} onClick={() => navigate(`/rtv/${r.rtvId}`)}>
                                            ...
                                            <td>
                                                <span style={{
                                                    ...
                                                    color: '#c2410c', fontSize: 11.5,
                                                    background: '#ffedd5', padding: '2px 8px', borderRadius: 4,
                                                }}>{r.rtvNumber}</span>
                                            </td>
                                            <td style={{ color: '#475569', fontSize: 12 }}>{fmtDate(r.rtvDate)}</td>
                                            <td style={{ maxWidth: 170, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                                {r.supplierName || <span className="muted">—</span>}
                                            </td>
                                            <td style={{ fontFamily: 'Courier New', fontSize: 11, color: '#166534', background: r.grnNumber ? '#dcfce7' : 'transparent', padding: r.grnNumber ? '2px 6px' : undefined, borderRadius: 4 }}>
                                                {r.grnNumber || <span className="muted" style={{ fontFamily: 'inherit', background: 'none' }}>—</span>}
                                            </td>
                                            ...
                                            <td className="r" style={{ fontFamily: 'monospace', color: r.exchangeRate && r.exchangeRate !== 1 ? '#7c3aed' : '#c2410c', fontSize: 12 }}>
                                                {fmt(r.totalAmount)}
                                            </td>
                                            <td className="r" style={{ fontFamily: 'monospace', fontWeight: 700, color: '#c2410c', fontSize: 12.5 }}>
                                                {fmt(r.totalAmountBase)}
                                            </td>
                                            <td style={{ fontSize: 11.5, color: '#64748b', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                {r.returnReason || <span className="muted">—</span>}
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                                {totals && (
                                    <tfoot>
                                        <tr style={{ background: '#f1f5f9', borderTop: '2px solid #cbd5e1', fontWeight: 700 }}>
                                            <td colSpan={10} style={{ textAlign: 'right', padding: '8px 10px', color: '#334155', fontSize: 12 }}>
                                                Totals — {totals.count} RTV{totals.count !== 1 ? 's' : ''}
                                            </td>
                                            <td />
                                            <td />
                                            <td style={{ textAlign: 'right', padding: '8px 6px', fontFamily: 'monospace', color: '#c2410c', fontWeight: 700 }}>{fmt(totals.totalAmountBase)}</td>
                                            <td />
                                        </tr>
                                    </tfoot>
                                )}
