...
const exportCsv = (rows) => {
    const headers = ['#', 'GRN Number', 'Date', 'Vendor', 'Job', 'PO Number', 'DO No', 'Invoice No', 'Status', 'Currency', 'Amount', 'Created By'];
    const esc = v => {
        if (v == null) return '';
        const s = String(v);
    ...
    const lines = [
        ...
        ...rows.map((r, i) => [
            ...
            r.vendorName, r.jobId, r.poNumber, r.doNo, r.invoiceNo,
            r.status, r.currencyShort, r.totalAmount, r.createdBy,
        ].map(esc).join(','))
    ];
    const blob = new Blob([lines.join('\n')], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `GRN_Report_${today()}.csv`; a.click();
    URL.revokeObjectURL(url);
};

// ── Status badge ────────────────────────────────────────────────────────
const StatusBadge = ({ status, getStatusConfig }) => {
    const cfg = statusBadgeCfg(getStatusConfig('GRN', status));
    return (
        <span style={{
...
const GrnReport = () => {
    ...
    const runReport = useCallback(async () => {
        ...
        try {
            const res  = await fetch(`${variables.API_URL}reports/grn?${p}`, { headers: authHeaders() });
    ...
    return (
        <div className="rpt-page">
            ...
            <div className="rpt-header">
                ...
                <div>
                    <div className="rpt-header-title">Goods Receipt Note Report</div>
                    <div className="rpt-header-sub">
                        {rows == null
                            ? 'Set filters and click Run Report'
                            : `${rows.length} GRN${rows.length !== 1 ? 's' : ''} found`}
                    </div>
                </div>
            ...
            {totals && (
                <div className="rpt-summary">
                    <div className="rpt-summary-item">
                        <div className="rpt-summary-label">Total GRNs</div>
            ...
            <div className="rpt-content">
                ...
                {!loading && rows !== null && rows.length === 0 && (
                    <div className="rpt-state">
                        <div className="rpt-state-icon">🔍</div>
                        <span>No GRNs match the selected filters.</span>
                    </div>
                )}
                            ...
                            <table className="rpt-table">
                                <thead>
                                    <tr>
                                        ...
                                        <Th col="currencyShort" label="Currency"     />
                                ...
                                <tbody>
                                    {paged.map((r, i) => (
                                        <tr key={r.grnId} onClick={() => navigate(`/grn/${r.grnId}`)}>
                                            ...
                                            <td style={{ fontSize: 11.5, color: '#475569', fontFamily: 'Courier New' }}>
                                            ...
                                            <td style={{ fontSize: 11.5, color: '#64748b' }}>
                                                {r.invoiceNo || <span className="muted">—</span>}
                                            </td>
                                            <td>
                                            ...
                                            <td className="r" style={{ fontFamily: 'monospace', fontWeight: 700, color: '#166534', fontSize: 12.5 }}>
                                                {fmt(r.totalAmount)}
                                            </td>
