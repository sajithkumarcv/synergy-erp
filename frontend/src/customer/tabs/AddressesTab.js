...
const makeEmpty = (customerId, addressTypes, countries) => ({
    customerAddressId: 0, customerId,
...
const AddressForm = ({ customerId, record, onSaved, onCancel }) => {
    ...
    const save = async () => {
        ...
        try {
            ...
            const res = await fetch(variables.API_URL + 'customer/address/save', { method: 'POST', headers: authHeaders(), body: JSON.stringify(payload) });
...
// ─── ADDRESSES TAB ───────────────────────────────────────────────
const AddressesTab = ({ customer, onRefresh, canEdit = true }) => {
    const { lookups, getVList } = useLookup();
    const { countries } = lookups;
    const addressTypes = getVList('Customer', 'AddressType');
    ...
    const load = useCallback(() => {
        fetch(variables.API_URL + 'customer/addresses/' + customer.customerId, { headers: authHeaders() })
            .then(r => r.json())
    ...
    return (
        <div className="tab-section">
            <div className="tab-toolbar">
                ...
                {!formRecord && canEdit && (
                    <button className="tab-btn-pri" style={{ background: '#0f766e' }} onClick={() => setForm(makeEmpty(customer.customerId, addressTypes, countries))}>
                        + Add Address
                    </button>
                )}
            </div>
                ...
                <table className="tab-table">
                    ...
                    <tbody>
                        {filtered.length === 0
                            ...
                            : filtered.map(a => (
                                <tr key={a.customerAddressId} className={formRecord?.customerAddressId === a.customerAddressId ? 'tab-row-editing' : ''}>
