...
// (no imports needed from invoiceConstants — local helpers below cover all formatting)
...
// ─────────────────────────────────────────────────────────────────────
const InvoiceDocumentsTab = ({ invoice }) => {
    const currentUser         = useCurrentUser();
    const { getStatusConfig } = useLookup();
    const fileRef = useRef(null);

    // canUploadDocs from DB config; fallback: locked when Cancelled
    const editable = getStatusConfig('INV', invoice?.status)?.canUploadDocs
    ...
    const load = useCallback(() => {
        setLoading(true);
        fetch(`${variables.API_URL}document/INV/${invoice.invoiceId}`, { headers: authHeaders() })
            .then(r => r.json())
    ...
    useEffect(() => {
        fetch(`${variables.API_URL}document/types/INV`, { headers: authHeaders() })
            .then(r => r.ok ? r.json() : [])
            .then(d => {
        ...
        <div className="jd-tab-body">
            ...
            {!editable && (
                <div style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 8, padding: '8px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12.5, color: '#475569' }}>This invoice is locked. Documents cannot be uploaded or deleted.</span>
                </div>
            )}
