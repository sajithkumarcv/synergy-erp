                    ...
                    <div className="pf-row">
                        <div className="pf-field pf-f2" style={{ position: 'relative' }}>
                            ...
                            {form.supplierId ? (
                                ...
                                <>
                                    <input className={`pf-input${errors.supplierId ? ' pf-input-err' : ''}`} value={suppSearch}
                                        onChange={e => setSuppSearch(e.target.value)} placeholder="Search by name or code…" autoComplete="off" />
                                    {errors.supplierId && <span className="pf-field-err">{errors.supplierId}</span>}
                                    {suppResults.length > 0 && (
                                        <div style={dropStyle}>
                                            {suppResults.map(s => (
                                                <div key={s.supplierId} style={dropItem} onClick={() => pickSupplier(s)}
