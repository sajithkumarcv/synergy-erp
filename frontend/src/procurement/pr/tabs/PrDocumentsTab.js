...
// ─────────────────────────────────────────────────────────────
const PrDocumentsTab = ({ pr }) => {
    const currentUser = useCurrentUser();
    const { getStatusConfig } = useLookup();
    const fileRef  = useRef(null);
    // canUploadDocs from DB flag; fallback: locked when Approved/Rejected/Cancelled
    const editable = getStatusConfig('PR', pr?.status)?.canUploadDocs
        ?? !['Approved', 'Rejected', 'Cancelled'].includes(pr?.status);
    ...
    const load = useCallback(() => {
        setLoading(true);
        fetch(`${variables.API_URL}document/PR/${pr.prId}`, { headers: authHeaders() })
            .then(r => r.json())
    ...
    useEffect(() => {
        fetch(`${variables.API_URL}document/types/PR`, { headers: authHeaders() })
            .then(r => r.ok ? r.json() : [])
            .then(d => {
        ...
        <div className="jd-tab-body">
            ...
            {!editable && (
                <div style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 8, padding: '8px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12.5, color: '#475569' }}>This PR is locked. Documents cannot be uploaded or deleted.</span>
                </div>
            )}
