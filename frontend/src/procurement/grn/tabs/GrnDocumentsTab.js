...
// ─────────────────────────────────────────────────────────────
const GrnDocumentsTab = ({ grn }) => {
    const currentUser = useCurrentUser();
    const { getStatusConfig } = useLookup();
    const fileRef = useRef(null);
    // canUploadDocs from DB flag; fallback: locked when Received/Cancelled
    const editable = getStatusConfig('GRN', grn?.status)?.canUploadDocs
        ?? !['Received', 'Cancelled'].includes(grn?.status);
    ...
    const load = useCallback(() => {
        setLoading(true);
        fetch(`${variables.API_URL}document/GRN/${grn.grnId}`, { headers: authHeaders() })
            .then(r => r.json())
    ...
    useEffect(() => {
        fetch(`${variables.API_URL}document/types/GRN`, { headers: authHeaders() })
            .then(r => r.ok ? r.json() : [])
            .then(d => {
    ...
    const upload = () => {
        ...
        fetch(`${variables.API_URL}document/GRN/${grn.grnId}`, {
            method: 'POST',
        ...
        <div className="jd-tab-body">
            ...
            {!editable && (
                <div style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 8, padding: '8px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12.5, color: '#475569' }}>This GRN is locked. Documents cannot be uploaded or deleted.</span>
                </div>
            )}
...
export default GrnDocumentsTab;
