...
const PoSrvTab = ({ po }) => {
    ...
    const load = useCallback(() => {
        setLoading(true);
        setError('');
        fetch(`${variables.API_URL}servicereceipt/search?poId=${po.poId}&pageSize=100&page=1`, { headers: authHeaders() })
            .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
            .then(d => setSrvs(d.data || []))
    ...
    if (loading) return (
        <div className="jd-tab-body">
            <div style={{ padding: 40, textAlign: 'center', color: '#64748b', fontSize: 13 }}>Loading service receipts…</div>
        </div>
    );
        ...
        <div className="jd-tab-body">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 12, color: '#64748b' }}>
                    {srvs.length} SRV{srvs.length !== 1 ? 's' : ''} linked to this PO
                </div>
            </div>
            ...
            {srvs.length === 0 ? (
                <div style={{ background: '#f8fafc', border: '1px dashed #cbd5e1', borderRadius: 8,
                    ...
                    No service receipts have been raised against this PO yet.
                </div>
            ) : (
                        ...
                        <tbody>
                            {srvs.map(s => (
                                <tr key={s.srvId}
                                    ...
                                    onClick={() => navigate(`/service-receipts/${s.srvId}`)}>
                                    <td>
                                        <span style={{
                                    ...
                                    <td style={{ fontSize: 12.5 }}>{fmtDate(s.srvDate)}</td>
                                    <td>
                                        {s.jobId
                                            ? <span style={{ fontFamily: 'Courier New', fontSize: 11,
                                                color: '#0f766e', background: '#ccfbf1',
                                                padding: '2px 6px', borderRadius: 4 }}>{s.jobId}</span>
                                            : <span style={{ color: '#94a3b8' }}>—</span>}
                                    </td>
                                    <td style={{ fontSize: 12, color: '#475569' }}>{s.supplierName || <span style={{ color: '#94a3b8' }}>—</span>}</td>
                                    <td style={{ textAlign: 'right', fontFamily: 'monospace', fontWeight: 600, color: '#1e40af' }}>
                                        {s.totalCost ? fmt(s.totalCost) : <span style={{ color: '#94a3b8' }}>—</span>}
                                    </td>
