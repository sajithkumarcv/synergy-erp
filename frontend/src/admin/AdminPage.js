...
const SECTIONS = [
    ...
    // ── Jobs ─────────────────────────────────────────────────────────────────
    {
        ...
        columns: [
            ...
            { key: 'isBudgetHeaderLinked', label: 'Budget Linked', render: r => badge(r.isBudgetHeaderLinked) },
            { key: 'isActive',    label: 'Active', render: r => badge(r.isActive) },
            { key: 'sortOrder',   label: 'Sort' },
        ],
        fields: [
            ...
            { name: 'isBudgetHeaderLinked', label: 'Budget Header Linked (jobs need a budget category)', type: 'checkbox' },
            { name: 'isActive',           label: 'Active',             type: 'checkbox' },
            { name: 'sortOrder',          label: 'Sort Order',         type: 'number' },
        ],
        saveMapper: (f, _user, isNew) => ({
            ...
            isBudgetHeaderLinked: !!f.isBudgetHeaderLinked,
            isActive:          f.isActive!==false,
            sortOrder:         parseInt(f.sortOrder)||0,
            isNew:             !f._originalId,
        }),
