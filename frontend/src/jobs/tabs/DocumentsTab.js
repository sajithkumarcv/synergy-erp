...
import { fmtDateTime, canEdit } from '../jobConstants';
...
const DocumentsTab = ({ job }) => {
    ...
    const load = useCallback(() => {
        setLoading(true);
        fetch(`${variables.API_URL}document/JOB/${encodeURIComponent(job.jobId)}`, { headers: authHeaders() })
            .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
            .then(d => setDocs(Array.isArray(d) ? d : []))
            .catch(e => console.error('Load documents:', e))
    ...
    useEffect(() => {
        fetch(`${variables.API_URL}document/types/JOB`, { headers: authHeaders() })
            .then(r => r.ok ? r.json() : [])
            .then(d => {
    ...
    const openDoc = (id, fileName) => {
        fetch(`${variables.API_URL}document/${id}/download`, { headers: authHeaders() })
            ...
            .then(blob => {
                ...
                a.target = '_blank';
                a.rel    = 'noreferrer';
                // PDFs open inline; images download with filename
    ...
    // Mandatory types not yet uploaded
        ...
        <div className="jd-tab-body">
            ...
            {!editable && (
                <div style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 8, padding: '8px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12.5, color: '#475569' }}>This job is locked. Documents cannot be uploaded or deleted.</span>
                </div>
            )}
            ...
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: 16, marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', flex: '0 0 180px' }}>
